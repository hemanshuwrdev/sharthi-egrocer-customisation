  ...
  methods: {
      ...
      axios__WEBPACK_IMPORTED_MODULE_2___default().get(this.$apiUrl + '/categories/product_count').then(function (response) {
        ...
        _this2.graphCategories.forEach(function (category) {
          if (category.product_count !== 0) {
            _this2.options2.labels.push(_this2.getDisplayName(category.name));
    ...
    getOrderStatus: function getOrderStatus() {
      ...
      Promise.all([axios__WEBPACK_IMPORTED_MODULE_2___default().get(this.$apiUrl + '/order_statuses'), axios__WEBPACK_IMPORTED_MODULE_2___default().get(this.$apiUrl + '/order_statuses/self_pickup')]).then(function (_ref) {
        ...
        var regular = regularRes.data.data || [];
        // Self-pickup statuses excluding IDs already in regular (1,7,8 are shared)
        var regularIds = new Set(regular.map(function (s) {
          return s.id;
        }));
        var selfPick = (selfPickRes.data.data || []).filter(function (s) {
          return !regularIds.has(s.id);
        });
        _this5.statuses = [].concat(_toConsumableArray(regular), _toConsumableArray(selfPick));
      })["catch"](function (error) {
        vm.isLoading = false;
    ...
    getLatestOrders: function getLatestOrders() {
      ...
      axios__WEBPACK_IMPORTED_MODULE_2___default().get(this.$apiUrl + '/orders', {
        ...
        if (data.status === 1) {
          ...
          _this6.orders = response.data.data.orders.map(function (o) {
            o.order_items = [];
    ...
    toggleOrder: function toggleOrder(itemOrRow) {
      ...
      if (!isShowing) {
        this.orders.forEach(function (order) {
          _this8.$set(order, '_showDetails', false);
        });
      }
      ...
      axios__WEBPACK_IMPORTED_MODULE_2___default().get(this.$apiUrl + '/orders/view/' + item.id).then(function (res) {
        ...
        item.itemsLoading = false;
        _this8.showError('Failed to load order items');
      });
    },
                ...
                [
                  _c("div", { staticClass: "card h-100" }, [
                    _c(
                      ...
                      {
                        staticClass:
                          "card-body p-2 p-sm-3 h-100 d-flex flex-column justify-content-center align-items-center text-center",
                      },
                      [
                        _c(
                          "div",
                              ...
                              status.status == "Ready for Pickup"
                                ? _c(
                                    ...
                                    [
                                      _vm._v(
                                        "\n                                        " +
                                          _vm._s(_vm.__("ready_for_pickup"))
                                      ),
                                    ]
                                  )
                                : _vm._e(),
                      ...
                      _c(
                        ...
                        [
                          _c(
                            ...
                            {
                              staticClass: "mb-2",
                              attrs: { cols: "12", md: "7", lg: "4" },
                            },
                            [
                              _c(
                                "label",
                                { staticClass: "box-title d-block mb-1" },
                              ...
                              _c(
                                ...
                                [
                                  ...
                                  _c(
                                    ...
                                    {
                                      staticClass:
                                        "btn btn-sm btn-danger ms-1 flex-shrink-0",
                                      on: {
                          ...
                          _c(
                            ...
                            {
                              staticClass:
                                "mb-2 d-flex align-items-end justify-content-center",
                              attrs: { cols: "12", md: "12", lg: "1" },
                            },
                            [
                              _c(
                                "button",
