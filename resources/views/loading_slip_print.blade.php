            ...
            @foreach($itemSummary as $index => $item)
                @php
                    ...
                    if (isset($item->secondary_unit_value) && $item->secondary_unit_value > 1) {
                        $displayName .= ' ' . (int)$item->secondary_unit_value . ' X ' . strtoupper($item->variant_name);
                    } else {
                ...
                <tr>
                    ...
                    <td class="center" style="font-size: 13px; font-weight: bold; color: #df2029;">{{ $boxQty }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <table>
        ...
        <tbody>
            @foreach($orders as $index => $order)
                <tr>
                    <td class="center" style="font-size: 14px; font-weight: bold; color: #df2029;">{{ $index + 1 }}</td>
                    <td>
                        <strong>{{ $order->party_name ?: ($order->shop_name ?: $order->user_name) }}</strong>
                        @if($order->city_zone)
                            <span style="color: #666; font-size: 10px; font-weight: bold; margin-left: 10px;">(Zone: {{ $order->city_zone }})</span>
                        @endif
                        <br><span style="font-size: 10.5px; color: #555; font-weight: normal;">{{ $order->customer_address }}</span>
                    </td>
                    <td class="center">#{{ $order->id }}</td>
                    <td class="right" style="font-weight: bold; font-size: 13px;">₹{{ number_format($order->final_total, 2) }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
            ...
            <div class="udaan-invoice">
                ...
                <div class="udaan-header">
                    <div class="udaan-logo-container">
                        ...
                        <div>
                            ...
                            <div style="font-size: 8px; color: #666; font-weight: bold; margin-top: -3px; letter-spacing: 0.5px;">B2B WHOLESALE DISTRIBUTOR</div>
                        </div>
                    </div>
                    
                    <div class="udaan-meta-grid">
                        <span class="udaan-meta-label">Invoice ID:</span>
                        <span class="udaan-meta-val">INV/{{ date('Y') }}/{{ str_pad($order->id, 6, '0', STR_PAD_LEFT) }}</span>
                ...
                <div class="udaan-address-columns">
                    ...
                    <div class="udaan-address-col">
                        <h4>BILL FROM:</h4>
                        <p><strong>{{ $seller->name ?? $app_name }}</strong></p>
                        <p>{{ $seller->street ?? 'Warehouse Sector 4' }}</p>
                        <p>{{ $seller->city_name ?? 'Bhuj' }}, {{ $seller->state ?? 'Gujarat' }}</p>
                        <p>GSTIN: {{ $seller->tax_number ?? '24AAECH3221K1ZV' }}</p>
                        <p>PAN: {{ $seller->pan_number ?? 'AAECH3221K' }}</p>
                        <p>FSSAI: 11223999000823</p>
                    </div>

                    <!-- Ship From -->
                    <div class="udaan-address-col">
                        <h4>Ship From:</h4>
                        <p><strong>{{ $seller->store_name ?? ($seller->name ?? $app_name) }}</strong></p>
                        <p>{{ $seller->street ?? 'Warehouse Sector 4' }}</p>
                        <p>{{ $seller->city_name ?? 'Bhuj' }}, {{ $seller->state ?? 'Gujarat' }}</p>
                        <p>Place of Supply: {{ strtoupper($seller->city_name ?? 'BHUJ') }}</p>
                    ...
                    <div class="udaan-address-col">
                        ...
                        <p><strong>{{ $order->party_name ?: ($order->shop_name ?: $order->user_name) }}</strong></p>
                        <p>{{ $order->customer_address }}</p>
                        <p>Mobile: {{ $order->mobile }}</p>
                        <p>GSTIN: {{ $order->customer_gst ?? '24AAEQH9411R1Z5' }}</p>
                    </div>
                    ...
                    <div style="text-align: right; display: flex; flex-direction: column; align-items: flex-end; justify-content: flex-start;">
                        ...
                        <span style="font-size: 7px; color: #666; margin-top: 4px; font-weight: bold; text-align: center; width: 65px; letter-spacing: 0.5px;">SCAN CODE</span>
                    </div>
                </div>
                        ...
                        @foreach($order->items as $itemIndex => $item)
                            ...
                            <tr>
                                <td>
                                    ...
                                    @endif
                                    <br><span style="color: #555; font-size: 8px;">Variant: {{ $item->variant_name }} | HSN: 110610</span>
                                </td>
                                <td style="text-align: center; font-weight: bold;">{{ number_format($item->quantity, 1) }}</td>
                ...
                <div class="udaan-totals-area">
                    <table class="udaan-totals-table">
                        <tr>
                            ...
                            <td style="text-align: right; font-weight: 500;">₹{{ number_format($totalNetTaxable, 2) }}</td>
                        </tr>
                        <tr>
                            <td style="text-align: left; color: #555;">Total Discount</td>
                            <td style="text-align: right; font-weight: 500;">₹{{ number_format(($order->total * $order->discount / 100) + $order->promo_discount, 2) }}</td>
                        </tr>
                        <tr>
                            <td style="text-align: left; color: #555;">Net Taxable Amount</td>
                            <td style="text-align: right; font-weight: bold;">₹{{ number_format($totalNetTaxable, 2) }}</td>
                        </tr>
                        <tr>
                        ...
                        @if($order->delivery_charge > 0)
                            <tr>
                                ...
                                <td style="text-align: right; font-weight: bold;">₹{{ number_format($order->delivery_charge, 2) }}</td>
                            </tr>
                        @endif
                ...
                <div class="udaan-footer-rules">
                    <div style="max-width: 70%;">
                        <p style="margin: 0 0 3px 0;"><strong>Terms of trade credit provided by {{ $seller->name ?? $app_name }}:</strong></p>
                        <p style="margin: 0 0 3px 0;">Bill amount to be paid in full. Payments by cash, UPI, or Bank Transfer.</p>
                    ...
                    <div style="text-align: right; min-width: 150px; display: flex; flex-direction: column; align-items: flex-end; justify-content: flex-end;">
                        <div style="font-family: 'Courier New', Courier, monospace; font-size: 11px; margin-bottom: 2px; color: #555; font-style: italic; font-weight: bold;">Ajay Shah</div>
                        <div style="border-top: 1px solid #000; width: 120px; text-align: center; padding-top: 3px; font-weight: bold; font-size: 8px;">Authorised Signatory</div>
                    </div>
                </div>
            </div>
        </div>
