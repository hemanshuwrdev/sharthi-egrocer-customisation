            ...
            <div class="page-title">
                <div class="row">
                    <div class="col-12 col-md-6 order-md-1 order-last">
                        <h3>{{ __('salesman_privacy_policy_and_term_conditions') }}</h3>
                    </div>
                    <div class="col-12 col-md-6 order-md-2 order-first">
                        <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><router-link to="/dashboard">{{ __('dashboard') }}</router-link></li>
                                <li class="breadcrumb-item active" aria-current="page">{{ __('salesman_privacy_policy_and_term_conditions') }}</li>
                            </ol>
                        </nav>
                    </div>
                </div>
                ...
                <form id="contact_us_form" method="post" enctype="multipart/form-data" @submit.prevent="saveRecord">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">{{ __('salesman_privacy_policy_and_term_conditions') }}</h4>
                        </div>
                        <div class="card-body">
                            <!-- Language Tabs -->
                            ...
                            <div class="form-group">
                                <div class="d-flex justify-content-between mb-2">
                                                <label>{{ __('privacy_policy') }} <i class="text-danger" v-if="language.is_default">*</i></label>
                                                <a :href="$baseUrl+'/salesman-privacy-policy?lang=' + language.code" v-b-tooltip.hover :title="__('privacy_policy')" target="_blank" class="btn btn-sm btn-primary"><i class="fa fa-eye"></i></a>
                                </div>
                                <editor
                                    :placeholder="__('privacy_policy')"
                                                v-model="translations[language.id].privacy_policy_salesman"
                                                :init="getEditorConfig()"
                                                :disabled="!$route.path.includes('/seller')"
                                />
                            </div>

                                        <h4 class="card-title mt-4">{{ __('update_term_conditions') }}</h4>
                            <div class="form-group">
                                <div class="d-flex justify-content-between mb-2">
                                                <label>{{ __('term_conditions') }} <i class="text-danger" v-if="language.is_default">*</i></label>
                                                <a :href="$baseUrl+'/salesman-terms-conditions?lang=' + language.code" v-b-tooltip.hover :title="__('terms_conditions')" target="_blank" class="btn btn-sm btn-primary"><i class="fa fa-eye"></i></a>
                                </div>
                                <editor
                                    :placeholder="__('terms_conditions')"
                                                v-model="translations[language.id].terms_conditions_salesman"
                                                :init="getEditorConfig()"
                                                :disabled="!$route.path.includes('/seller')"
                                            />
                                        </div>
...
export default {
    ...
    methods: {
        ...
        loadPolicyData() {
            axios.get(this.$apiUrl + '/privacy_policy_salesman').then((response) => {
                ...
                this.languages.forEach(language => {
                    ...
                    if (privacyPolicyData[language.id]) {
                        this.$set(this.translations[language.id], 'privacy_policy_salesman', privacyPolicyData[language.id]);
                    }
                    if (termsConditionsData[language.id]) {
                        this.$set(this.translations[language.id], 'terms_conditions_salesman', termsConditionsData[language.id]);
                    }
                });
            });
        },
        ...
        saveRecord: function() {
            ...
            let url = this.$apiUrl + '/privacy_policy_salesman/save';
