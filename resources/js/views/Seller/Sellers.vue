<template>
    <div class="list-page">
        <div class="page-head">
            <h3 class="page-head-title">{{ __('sellers') }}</h3>
            <router-link to="/sellers/create" class="btn btn-primary list-add-btn d-inline-flex align-items-center gap-2 text-nowrap" v-b-tooltip.hover title="Add New Seller" v-if="$can('seller_create')">
                            ...
                            <template #cell(categories_array)="row">
                                ...
                                <template #cell(actions)="row">
                                    <div class="list-actions">
                                        <router-link :to="{ name: 'EditSeller',params: { id: row.item.id, record : row.item }}" v-b-tooltip.hover title="Edit" class="list-action-btn is-edit" v-if="$can('seller_update')" v-b-tooltip.hover :title="__('edit')">
                                            <i class="fa fa-pencil-alt"></i>
                                        </router-link>
                                        <button class="list-action-btn is-delete" @click="deleteSeller(row.index,row.item.id)" v-if="$can('seller_delete')" v-b-tooltip.hover :title="__('delete')">
